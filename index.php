...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($user) && ($user->auth== 'manual' || $user->auth == 'email')) {
        complete_user_login($user);
        // Get system context
        $context = context_system::instance();
        $userss = get_user_roles($context, $user->id);
        // Get all roles assigned to user at system level
        ...
        if (is_siteadmin($user)) {
            redirect(new moodle_url('/admin/search.php'));
        } elseif (user_has_role_assignment($user->id, 5,0)) { // Assuming 5 = student
            redirect(new moodle_url('/dashboard/'));
        }elseif (user_has_role_assignment($user->id, 3,0)) { // Assuming 5 = student
             redirect(new moodle_url('/my/courses.php'));
        }else{
            redirect(new moodle_url('/my/'));
        }
        // print_r(user_has_role_assignment($user->id, 5));
        //    echo "<pre>";
        // print_r($user);
        // echo "hssh";
        // print_r(is_siteadmin($user));
        // exit;
        ...
        if (filter_var($login_input, FILTER_VALIDATE_EMAIL)) {
            $user = $DB->get_record('user', ['email' => $login_input, 'deleted' => 0]);
            if ($user) {
                $login_input = $user->username; // Use the username for login
            }
            $user = authenticate_user_login($login_input, $password);
            if (!empty($user) && ($user->auth== 'manual' || $user->auth == 'email')) {
                complete_user_login($user);
                // Get system context
                $context = context_system::instance();
                ...
                if (is_siteadmin($user)) {
                 ...
                 $msg_login_invalid =  "<font color='red'>Invalid login credential. Please try again...</font>";
            }
        }else{
             $msg_login_invalid =  "<font color='red'>Invalid login credential. Please try again...</font>";
        }
...
<body>
            ...
            <div class="row gx-3 gy-md-0 gy-3">
                <div class="col-md-6 col-lg-5">
                    ...
                    <div class="row g-4">
                        ...
                        <!-- <div class="col-md-6 col-6">
                             <a href ="https://www.azschoolofmedicalassistant.com/student-portal" >
                ...
                <div class="col-md-6 col-lg-5 SCornLoginSecCo">
                    ...
                    <div class="SCCourseHead">
                        ...
                        <h3>Login in Echocardiogram Program</h3>
                       
                    </div>
                     <span style = "<?php echo $stle?>"><?php echo $msg_login_invalid;?></span>
                    <div class="row g-5"> 
                        
                        <form method="post" action="<?php echo $CFG->wwwroot; ?>/student-portal.php" onsubmit="return validateLoginForm();">
                            <div class="row gy-2">
                                <div class="formGroup contactFormGroup">
                                    <label for="yourName" class="form-label">Email Address</label>
                                    <input type="text" name="username" placeholder="Username" required >
                                </div>
                                   <div class="formGroup contactFormGroup position-relative">
    ...
    <span onclick="togglePassword()"
          style="position:absolute; right:20px; top:70%; transform:translateY(-50%); cursor:pointer;"
          id="toggleIcon">

        <!-- Eye SVG -->
        <svg id="eyeOpen" xmlns="http://www.w3.org/2000/svg"
             width="20" height="20" viewBox="0 0 24 24"
             fill="none" stroke="#4E5D8D"
    ...
    <input type="hidden" name="logintoken" value="<?php echo $logintoken; ?>" />
                            ...
                            <button type="submit" class="submitContactBtn w-100">Login <span>
                                    <svg width="54" height="54" viewBox="0 0 54 54" fill="none">
                                        <path
  ...
  ?>
    ...
    if (passwordField.type === "password") {
        ...
        eyeIcon.innerHTML = `
            ...
            <path d="M9.88 4.24A10.94 10.94 0 0112 4c7 0 11 8 11 8a21.86 21.86 0 01-5.17 6.58"/>
            <path d="M6.53 6.53A21.86 21.86 0 001 12s4 8 11 8a10.94 10.94 0 005.76-1.67"/>
        `;
    } else {
